def Mul = fn a, b -> a * b, Double = Mul(2);

let rec Fib = fn n -> if n < 2 then n else Fib(n - 1) + Fib(n - 2) in Fib(87);
...
let rec Ack = fn m, n -> if m == 0 then n + 1 else if n == 0 then Ack(m - 1, 1) else Ack(m - 1, Ack(m, n - 1)) in Ack(3, 3);
...
let Y = fn f -> (fn x -> x(x))(fn x -> f(x(x))),
    NonRecGcd = fn self, a, b -> if b == 0 then a else self(b, a % b),
    Gcd = fn a, b -> Y(NonRecGcd)(a, b)
        in Gcd(56, 88);


let a = 2, b = a * a in b + a;